...
async function readFileFromDirs(urlPath: string): Promise<Buffer> {
  const rootFilePath = resolve(join(ROOT_DIR, urlPath));
  const srcFilePath = resolve(join(SRC_DIR, urlPath));
  const publicFilePath = resolve(join(PUBLIC_DIR, urlPath));

  try {
    return await readFile(rootFilePath);
  } catch {
    try {
      return await readFile(srcFilePath);
    } catch {
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (ext === ".html") {
      data = Buffer.from(injectHMRScript(data.toString("utf8")));
    }
